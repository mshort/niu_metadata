<xsl:stylesheet xmlns:marc="http://www.loc.gov/MARC21/slim" xmlns:mods="http://www.loc.gov/mods/v3" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="xlink marc" version="1.0">
...
Revision 1.78 - Fixed 852 mapping <shelfLocation> was changed to <mods:shelfLocator> - 2012/05/07 tmee
...
Revision 1.48 - Aquifer revision 1.27 - Added mapping of 242 second indicator (for nonfiling characters) to <mods:titleInfo><nonSort > subelement  2007/08/08 tmee/dlf
...
Revision 1.24 - Mapped 852 subfields $h, $i, $j, $k, $l, $m, $t to <shelfLocation> and 852 subfield $u to <mods:physicalLocation> with @xlink 2008/09/17 tmee
...
Revision 1.02 - Added Log Comment  2003/03/24 19:37:42  ckeith
	...
	<xsl:template match="/">
		<mods:mods xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="3.5" xsi:schemaLocation="http://www.loc.gov/mods/v3 http://www.loc.gov/standards/mods/v3/mods-3-5.xsd">
...
-->
		...
		<mods:originInfo eventType="publication">
			...
			<xsl:for-each select="marc:datafield[(@tag=260 or @tag=264 or @tag=250) and marc:subfield[@code='a' or code='b' or @code='c' or code='g']]">
			...
			<xsl:for-each select="marc:datafield[@tag=264 and @ind2=1]/marc:subfield[@code='b' or @code='c' or @code='g']">
				<xsl:choose>
					<xsl:when test="@code='b'">
						<mods:publisher>
							<xsl:call-template name="chopPunctuation">
			...
			<xsl:if test="$controlField008-6='t'">
				<xsl:if test="$controlField008-11-14">
					<xsl:choose>
						<xsl:when test="contains(marc:datafield[@tag=264 and @ind2=1]/marc:subfield[@code='c'],'[')">
							<mods:dateIssued keyDate="yes" encoding="w3cdtf" qualifier="inferred">
								<xsl:value-of select="$controlField008-7-10"/>
							</mods:dateIssued>
		...
		<xsl:for-each select="marc:datafield[@tag=382]/marc:subfield[@code='a']">
...
-->
		...
		<mods:relatedItem type="host">
			...
			<mods:location>
				<mods:url>http://rbsc.lib.niu.edu/islandora/object/rbsc:schreiner</mods:url>
			</mods:location>
		</mods:relatedItem>
		...
		<mods:recordInfo>
			...
			<mods:recordOrigin>Converted from MARCXML to MODS version 3.5 using MARC21slim2MODS3-5.xsl (Revision 1.106 2014/12/19)</mods:recordOrigin>
	...
	<xsl:template name="z3xx880">
		...
		<xsl:variable name="x336">
			<xsl:if test="@tag='336' and marc:subfield[@code='6']">
				<xsl:variable name="sf06336" select="normalize-space(child::marc:subfield[@code='6'])"/>
				<xsl:variable name="sf06336a" select="substring($sf06336, 1, 3)"/>
				<xsl:variable name="sf06336b" select="substring($sf06336, 5, 2)"/>
				<xsl:variable name="sf06336c" select="substring($sf06336, 7)"/>
				<xsl:value-of select="$sf06336b"/>
			</xsl:if>
		</xsl:variable>
		...
		<xsl:choose>
			<xsl:when test="$x351!='' and $x300!=''">
				<xsl:attribute name="altRepGroup">
					<xsl:value-of select="concat($x351, $x300, $x336, $x337, $x338)"/>
				</xsl:attribute>
			</xsl:when>
			<xsl:when test="$x351!=''">
				<xsl:attribute name="altRepGroup">
...
-->
	...
	<xsl:template name="createGenreFrom655">
		<xsl:choose>
			<xsl:when test="marc:subfield[@code='2']='aat'">
				<mods:genre>
					<xsl:attribute name="authority">
						<xsl:value-of select="marc:subfield[@code='2']"/>
					</xsl:attribute>
					<xsl:if test="contains(marc:subfield[@code='a'], 'Sheet music')">
						<xsl:attribute name="authorityURI">http://vocab.getty.edu/aat</xsl:attribute>
						<xsl:attribute name="valueURI">http://vocab.getty.edu/aat/300026430</xsl:attribute>
					</xsl:if>
					<xsl:value-of select="translate(translate(marc:subfield[@code='a'], $uppercase, $smallcase),'.','')"/>
				</mods:genre>
			</xsl:when>
			<xsl:otherwise>
				<mods:genre authority="marcgt">
	...
	<xsl:template name="createNoteFrom382">
		<mods:note type="medium of performance">
			<xsl:call-template name="xxx880"/>
			<xsl:call-template name="uri"/>
			<xsl:value-of select="."/>
		</mods:note>
	...
	<xsl:template name="createNoteFrom500">
		<xsl:choose>
			<xsl:when test="contains(marc:subfield[@code='a'], 'Cover illustration: ')">
				<mods:note type="cover illustration">
					<xsl:call-template name="xxx880"/>
					<xsl:call-template name="uri"/>
					<xsl:value-of select="substring-after(marc:subfield[@code='a'],'Cover illustration: ')"/>
				</mods:note>
			</xsl:when>
			<xsl:when test="contains(marc:subfield[@code='a'], 'First line of lyrics: ')">
				<mods:note type="first line of lyrics">
					<xsl:call-template name="xxx880"/>
					<xsl:call-template name="uri"/>
					<xsl:value-of select="substring-after(marc:subfield[@code='a'],'First line of lyrics: ')"/>
				</mods:note>
			</xsl:when>
			<xsl:when test="contains(marc:subfield[@code='a'], 'First line of chorus: ')">
				<mods:note type="first line of chorus">
					<xsl:call-template name="xxx880"/>
					<xsl:call-template name="uri"/>
					<xsl:value-of select="substring-after(marc:subfield[@code='a'],'First line of chorus: ')"/>
				</mods:note>
			</xsl:when>
			<xsl:otherwise>
				<mods:note>
...
-->
	...
	<xsl:template name="createSubTopFrom650">
		<mods:subject>
			<xsl:call-template name="xxx880"/>
			<xsl:call-template name="subjectAuthority"/>
			<xsl:if test="marc:subfield[@code='2']='fast'">
				<xsl:attribute name="authorityURI">http://id.worldcat.org/fast</xsl:attribute>
				<xsl:attribute name="valueURI">
					<xsl:text>http://id.worldcat.org/fast/</xsl:text>
					<xsl:value-of select="substring-after(marc:subfield[@code='0'], '(OCoLC)fst0')"/>
				</xsl:attribute>
			</xsl:if>
			<mods:topic>
				<xsl:call-template name="chopPunctuation">

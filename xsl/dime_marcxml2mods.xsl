<xsl:stylesheet xmlns="http://www.loc.gov/mods/v3" xmlns:marc="http://www.loc.gov/MARC21/slim" xmlns:mods="http://www.loc.gov/mods/v3" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="xlink marc" version="1.0">
	...
	<xsl:template name="marcRecord">
		...
		<xsl:for-each select="marc:datafield[@tag=880]">
			...
			<xsl:choose>
				...
				<xsl:when test="$hit='264'">
					<mods:originInfo eventType="publication">
						<xsl:call-template name="scriptCode"/>
						<xsl:for-each select="../marc:datafield[@tag=264 and marc:subfield[@code='a' or code='b' or @code='c' or code='g']]">
							<xsl:call-template name="z2xx880"/>
						</xsl:for-each>
						<xsl:if test="marc:subfield[@code='a']">
							<mods:place>
		...
		<mods:relatedItem type="host">
			...
			<mods:location>
				<url>http://dimenovels.lib.niu.edu</url>
			</mods:location>
		</mods:relatedItem>
		...
		<mods:accessCondition type="rightsstatement" xlink:href="https://creativecommons.org/publicdomain/mark/1.0/">No Copyright - Public Domain</mods:accessCondition>
		...
		<mods:recordInfo>
			...
			<mods:recordOrigin>Converted from MARCXML to MODS version 3.5 using MARC21slim2MODS3-4.xsl, modified for MODS version 3.5 and Nickels and Dimes by Matthew Short</mods:recordOrigin>
		...
		<xsl:variable name="x264">
			<xsl:choose>
				...
				<xsl:when test="@tag='250' and ../marc:datafield[@tag='264']/marc:subfield[@code='6']">
					<xsl:variable name="sf06264" select="normalize-space(../marc:datafield[@tag='264']/marc:subfield[@code='6'])"/>
					<xsl:variable name="sf06264a" select="substring($sf06264, 1, 3)"/>
					<xsl:variable name="sf06264b" select="substring($sf06264, 5, 2)"/>
					<xsl:variable name="sf06264c" select="substring($sf06264, 7)"/>
					<xsl:value-of select="$sf06264b"/>
	...
	<xsl:template name="createGenreFrom655">
		<xsl:if test="not(marc:subfield[@code='a']='Dime novels.') and not(marc:subfield[@code='a']='Dime novels')and not(marc:subfield[@code='a']='Nickel weeklies.') and not(marc:subfield[@code='a']='Nickel weeklies.')and not(marc:subfield[@code='a']='Penny dreadfuls.') and not(marc:subfield[@code='a']='Penny dreadfuls') and not(marc:subfield[@code='a']='Story papers.')">
			<mods:genre authority="marcgt">
				<xsl:attribute name="authority">
					<xsl:choose>
						<xsl:when test="@ind2='0'">
